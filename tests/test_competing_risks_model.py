...
def create_test_data(N=100):
    ...
    data.loc[data["transition"] == 1, "T"] = 1 + np.round(np.random.exponential(scale=2,
                                                                                size=np.sum(data["transition"] == 1)))
    data.loc[data["transition"] == 2, "T"] = 1 + np.round(np.random.exponential(scale=4,
                                                                                size=np.sum(data["transition"] == 2)))
...
def test_break_ties_by_adding_epsilon(crm=None):
    ...
    correct_t_after = [0.0, 0.09507143, 1.0, 2.07319939, 2.05986585, 3.0, 4.01560186, 4.01559945]
...
def test_fit_event_specific_model(df=None, crm=None):
    ...
    cox_model1 = crm._fit_event_specific_model(event_of_interest=1, df=df, duration_col="T", event_col="transition",
                                               cluster_col="id", verbose=2)

    # Fit second transition
    cox_model2 = crm._fit_event_specific_model(event_of_interest=2, df=df, duration_col="T", event_col="transition",
                                               cluster_col="id", verbose=2)
...
def test_extract_necessary_attributes(crm=None, df=None):
    ...
    if df is None:
        df = create_test_data(N=1_000)
    cox_model = crm._fit_event_specific_model(event_of_interest=1, df=df, duration_col="T", event_col="transition",
                                              cluster_col="id", verbose=0)
